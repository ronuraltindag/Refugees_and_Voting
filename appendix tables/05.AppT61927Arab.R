...
arab1927 <- read.csv('../data/arab1927.csv')
arab1927$Arab1927Pct[is.na(arab1927$Arab1927Pct)==TRUE] <-0 
 
#arab1927$Arab1927Pct[arab1927$Province=='hatay'] <- arab1927$Arab1965Pct[arab1927$Province=='hatay']

df2 <- merge(df2,arab1927, by='Province') 
...
df3$iv <- (df3$Arab1927Pct*df3$TotNbRefIn)/1000000  
...
t3.func <- function(x) {
  ...
  out1 <- felm(data=df.reg, formula= y ~  Age +  tRef1
               | idP + ym +  Eth + Educ + Sunni + Religious + Female + Area + income + missing.Age | 0 | idP)
  out2 <- felm(data=df.reg, formula= y ~  Age 
               | idP + ym +  Eth + Educ + Sunni + Religious + Female + Area + income + missing.Age | (tRef1~iv) | idP)
  s0 <-  round(out1$N,0)
  s1 <-  round(out1$coefficients[2,1],2)
  s2 <-  round(out1$cse[2],2) 
  s3 <-  round(out1$cpval[2],3)
  s4 <-  round(out2$coefficients[2,1],2)
  s5 <-  round(out2$cse[2],2)
  s6 <-  round(out2$cpval[2],3)
  s7 <-  round(mean(df.reg$y, na.rm=TRUE),2) 
  s8 <-  round(out2$stage1$coefficients[2,1],2)
  s9 <-  round(out2$stage1$cse[2],2)
  s10 <- round(out2$stage1$cpval[2],3)
  s11 <- round(out2$stage1$iv1fstat$tRef1[5],2)
  s.comb <- rbind(s0,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11)
  return(s.comb)
}
out <- lapply(1:(length(y.list)), t3.func) 
Arab1927PanelA <- as.data.frame(do.call(cbind, out))
...
t3.func <- function(x) {
  ...
  out1 <- felm(data=df.reg, formula= y ~  Under40Pct + FemalePct + PrimaryPct + tRef1
               | idP + ym| 0 | idP)
  out2 <- felm(data=df.reg, formula= y ~  Under40Pct + FemalePct + PrimaryPct 
               | idP + ym|  (tRef1~iv) | idP)
  s0 <-  round(out1$N,0)
  s1 <-  round(out1$coefficients[4,1],2)
  s2 <-  round(out1$cse[4],2) 
  s3 <-  round(out1$cpval[4],3)
  s4 <-  round(out2$coefficients[4,1],2)
  s5 <-  round(out2$cse[4],2)
  s6 <-  round(out2$cpval[4],4)
  s7 <-  round(mean(df.reg$y, na.rm=TRUE),2) 
  s8 <-  round(out2$stage1$coefficients[4,1],2)
  s9 <-  round(out2$stage1$cse[4],2)
  s10 <- round(out2$stage1$cpval[4],4)
  s11 <- round(out2$stage1$iv1fstat$tRef1[5],2)
  s.comb <- rbind(s0,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11)
  return(s.comb)
}
out <- lapply(1:(length(y.list)), t3.func) 
Arab1927PanelB <- as.data.frame(do.call(cbind, out))
