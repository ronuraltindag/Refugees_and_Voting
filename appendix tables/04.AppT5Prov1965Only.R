...
p.list.1965 <- c('osmaniye','batman','sirnak','yalova','kilis','kirikkale','karaman', 
                 'igdir','ardahan','bartin','karabuk','aksaray','gumushane','bayburt','duzce')
...
df1 <- df1 %>%
  select(var.set) %>%
  filter(!Province %in% p.list.1965) 
...
Prov1965PanelA <- as.data.frame(do.call(cbind, out))
...
out <- lapply(1:(length(y.list)), t3.func) 
Prov1965PanelB <- as.data.frame(do.call(cbind, out))
